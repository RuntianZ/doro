...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train,
              val_dataset=None, test_datasets=None):
    if dataset['verbose']:
    ...
    for batch in iterator:
        ...
        if train and config.evaluate_steps is not None and batch_idx % config.evaluate_steps == 0:
            if val_dataset is not None:
                run_epoch(algorithm, val_dataset, general_logger, epoch, config, train=False)
            if test_datasets is not None:
                for test_dataset in test_datasets:
                    run_epoch(algorithm, test_dataset, general_logger, epoch, config, train=False)
            algorithm.train()

    results, results_str = dataset['dataset'].eval(
        torch.cat(epoch_y_pred),
        torch.cat(epoch_y_true),
        (torch.cat(epoch_metadata), algorithm))
